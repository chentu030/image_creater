...
export const summarizeToMemory = async (text, mode = 'single') => {
  ...
  const prompt = mode === 'topic'
    ? `你是記憶整理助手。以下是一段完整的對話記錄，請整理成「重點筆記」，條列式，必須保留：
- 具體的結論、決定、數據、名稱
- 提到的具體方法、工具、技術細節
- 創意方向的具體描述（不要只寫「討論了創意方向」，要寫「決定用XX風格做YY」）
- 可延伸的具體想法

規則：不要抽象化，保留關鍵細節。不要寫開頭語。控制在 300 字以內。

對話記錄：
${text}`
    : `你是記憶整理助手。以下是一則 AI 回覆，請整理成條列式重點筆記。

規則：
- 保留所有具體的建議、名稱、數字、方法
- 不要抽象概括（❌「提供了多個建議」→ ✅「建議用A方法、B工具、C風格」）
- 不要寫開頭語，直接給重點
- 控制在 200 字以內
